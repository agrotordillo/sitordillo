document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("input", (event) => {
    ...
    const wrapper = event.target.closest(".producto-search");
    if (!wrapper) return;
  ...
  function mostrarResultados(items, input, hiddenInput, resultsEl) {
    ...
    items.forEach((item) => {
      ...
      btn.innerHTML = `<span class="font-mono text-xs text-gray-400">${item.folio}</span> · ${item.nombre} <span class="text-gray-400">(${item.sku})</span>`;
