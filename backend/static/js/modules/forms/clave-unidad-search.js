document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("input", (event) => {
    if (!event.target.matches(".clave-unidad-search-input")) return;
    const input = event.target;
    const wrapper = input.closest(".clave-unidad-search");
    if (!wrapper) return;
  ...
  document.addEventListener("click", (event) => {
    document.querySelectorAll(".clave-unidad-search-results:not(.hidden)").forEach((el) => {
      const wrapper = el.closest(".clave-unidad-search");
      if (wrapper && !wrapper.contains(event.target)) el.classList.add("hidden");
    });
  });

  async function buscar(wrapper, input) {
    const url = wrapper.dataset.claveUnidadSearchUrl;
    const resultsEl = wrapper.querySelector(".clave-unidad-search-results");
    ...
    try {
      ...
      if (window.App?.isDev) console.error("[clave-unidad-search]", err);
    }
  }
  ...
  function mostrarResultados(items, input, hiddenInput, resultsEl) {
    ...
    items.forEach((item) => {
      ...
      btn.addEventListener("click", () => {
        ...
        input.value = `${item.clave} · ${item.nombre}`;
        resultsEl.classList.add("hidden");
      });
