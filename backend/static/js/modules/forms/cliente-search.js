document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("input", (event) => {
    if (!event.target.matches(".cliente-search-input")) return;
    const input = event.target;
    const wrapper = input.closest(".cliente-search");
    if (!wrapper) return;
  ...
  document.addEventListener("click", (event) => {
    document.querySelectorAll(".cliente-search-results:not(.hidden)").forEach((el) => {
      const wrapper = el.closest(".cliente-search");
      if (wrapper && !wrapper.contains(event.target)) el.classList.add("hidden");
    });
  });

  async function buscar(wrapper, input) {
    const url = wrapper.dataset.clienteSearchUrl;
    const resultsEl = wrapper.querySelector(".cliente-search-results");
    ...
    try {
      ...
      if (window.App?.isDev) console.error("[cliente-search]", err);
    }
  }
  ...
  function mostrarResultados(items, input, hiddenInput, resultsEl) {
    ...
    items.forEach((item) => {
      ...
      btn.addEventListener("click", () => {
        ...
        input.value = item.rfc ? `${item.rfc} · ${item.nombre}` : item.nombre;
        resultsEl.classList.add("hidden");
      });
